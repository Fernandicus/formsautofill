...
// Allowing a delayed yield so the UI can paint the status overlay
...
export const usePdfProcessing = ({ groups, saveScrapedFields }: UsePdfProcessingProps) => {
  ...
  const processPdf = useCallback(async (file: File) => {
    try {
      ...
      dispatch({ type: 'MAPPING_COMPLETE', payload: { mappings: allMappings, pdfLanguage: detectedLanguage } });
    } catch (error) {
