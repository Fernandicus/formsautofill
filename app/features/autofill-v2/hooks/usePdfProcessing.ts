...
export const usePdfProcessing = ({ groups, saveScrapedFields }: UsePdfProcessingProps) => {
  ...
  const processPdf = useCallback(async (file: File) => {
    try {
      ...
      const markedBase64 = await generateMarkedPdfBase64(file, pdfFields);
      const { mappings: generatedMappings, detectedLanguage } = await mapFieldsWithGemini(pdfFields, flattenedFields, markedBase64);
